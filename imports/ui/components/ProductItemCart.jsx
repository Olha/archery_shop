...
export default class ProductItemCart extends React.Component {
	...
	removeFromCart(product) {
		...
		let response = removeCartItem.call(cartItem, displayError);
		if (response) {
		...
		return (
			<tr className='cart_item'>
				...
				<td className='product-thumbnail'>
					<a href='#' className='attachment-shop_thumbnail wp-post-image'><img
						src='http://placehold.it/58x63' alt=''/></a>
				</td>
				<td className='product-name'>
					<a
						href={FlowHelpers.pathFor(`/lists/${ product.listId }/products/${ product._id }`)}
						title={product.name}>
						{product.name}
					</a>
				</td>
